<template>
  <article class="card">
    <div class="card-info-hover">
      <svg @click="addFavoriteMealToUser" class="card-favorite" viewBox="0 0 24 24">
        <path
          fill="#000000"
...
import {createFavoriteMeal} from "@/services/meals.js";
    ...
    async addFavoriteMealToUser(){
      ...
      onAuthStateChanged(auth, async (user) => {
        if (user) {
          ...
          const newMeal = {...this.meal,userId}
          const res = await createFavoriteMeal(newMeal);
          console.log(res);
        }
      });
